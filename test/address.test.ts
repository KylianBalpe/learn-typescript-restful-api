...
describe("POST /api/contactId/addresses", () => {
  ...
  it('should be able to create address', async () => {
    ...
    expect(response.body.data.city).toBe("Sleman");
    expect(response.body.data.province).toBe("DI Yogyakarta");
    expect(response.body.data.country).toBe("Indonesia");
    expect(response.body.data.postal_code).toBe("55555");
  });
  ...
  it('should be reject create new address if request is contact is not found', async () => {
    const contact = await ContactTest.get();
    const response = await supertest(web)
...
describe("PUT /api/contacts/:contactId/addresses/:addressId", () => {
  ...
  it('should be able to update address', async () => {
    ...
    expect(response.body.data.city).toBe("Sleman update");
    expect(response.body.data.province).toBe("Update Yogyakarta");
    expect(response.body.data.country).toBe("Indonesia update");
    expect(response.body.data.postal_code).toBe("77777");
  });
